<template>
  <div class="popup-verification-need-docs">
    ...
    <span
      class="popup-status"
      :class="`-${verificationData.risk_acceptance_docs}`"
      v-text="getDocStatus"
    />
    <div class="popup-verification-need-docs-upload_block">
      <div class="popup-verification-need-docs-upload_block-container">
        <div class="popup-verification-need-docs-upload_block-container-icon">
          <lfx-icon-waiting v-if="verificationData.risk_acceptance_docs === 'process'" class="popup-verification-need-docs-upload_block-container-icon-flag" />
          <lfx-icon-error-fill v-else-if="verificationData.risk_acceptance_docs === 'rejected'" class="popup-verification-need-docs-upload_block-container-icon-flag" />
          <lfx-icon-success-fill v-else-if="verificationData.risk_acceptance_docs === 'verified'" class="popup-verification-need-docs-upload_block-container-icon-flag" />
...
export default {
  name: 'PopupVerificationRiskAcceptance',
  ...
  computed: {
    ...
    texts () {
      const result = this.info.filter((item, key) => [1, 3].includes(key))
    ...
    button () {
      const result = this.info.find(item => item.type === 'button')
    ...
    documentStatus () {
      return this.$store.getters['dictionaries/getters/handler/getRiskAcceptanceStatus']
    },
    getDocStatus () {
      return this.getUploadDocStatus(this.verificationData.risk_acceptance_docs)
    }
  },
  watch: {
    file: {
      handler (newValue) {
        if (newValue && newValue.name) {
          ...
          this.$store.dispatch('profile/actions/handler/uploadVerificationDocuments', { type: 'risk_acceptance', file: newValue })
          this.file = null
        }
      },
      deep: true
  ...
  methods: {
    ...
    uploadVerDocInfo () {
      this.$store.dispatch('profile/actions/handler/uploadVerificationDocumentsInfo', 'risk-acceptance')
    }
  }
}
</script>
