<template>
  <div class="popup-verification-need-docs">
    ...
    <span
      class="popup-status"
      :class="`-${verificationData.address_docs}`"
      v-text="getDocStatus"
    />
    <div class="popup-verification-need-docs-upload_block">
      <div class="popup-verification-need-docs-upload_block-container">
        <div class="popup-verification-need-docs-upload_block-container-icon">
          <lfx-icon-waiting v-if="verificationData.address_docs === 'process'" class="popup-verification-need-docs-upload_block-container-icon-flag" />
          <lfx-icon-error-fill v-else-if="verificationData.address_docs === 'rejected'" class="popup-verification-need-docs-upload_block-container-icon-flag" />
          <lfx-icon-success-fill v-else-if="verificationData.address_docs === 'verified'" class="popup-verification-need-docs-upload_block-container-icon-flag" />
    ...
    <span
      ...
      v-text="documentStatus === 'success' ? $t('Документ успешно загруженн') : $t('Произошла ошибка при загрузке файла')"
    />
...
export default {
  name: 'PopupVerificationEditAddress',
  components: { LfxIconSuccessFill, LfxIconErrorFill, LfxIconDocument, LfxIconWaiting, LfxIconAdd },
  data () {
    return {
      file: null
    }
  ...
  computed: {
    ...
    getDocStatus () {
      return this.getUploadDocStatus(this.verificationData.address_docs)
    }
  },
  watch: {
    file: {
      handler (newValue) {
        if (newValue && newValue.name) {
          ...
          this.$store.dispatch('profile/actions/handler/uploadVerificationDocuments', { type: 'address', file: newValue })
          this.file = null
        }
      },
      deep: true
  ...
  methods: {
    ...
    uploadVerDocInfo () {
      this.$store.dispatch('profile/actions/handler/uploadVerificationDocumentsInfo', 'address')
    }
  }
}
</script>
